...
<table width="680" class="tbl_global">
    ...
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Sites without Reseller")?></th>
        <td><?php if ($res_sites->numRows() == 0) echo "OK"; else while ($row = $res_sites->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[sitename]<br />"; $errors++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Domains without Sites")?></th>
        <td><?php if ($res_domains->numRows() == 0) echo "OK"; else while ($row = $res_domains->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[domainname]<br />"; $errors++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Users without Sites")?></th>
        <td><?php if ($res_users->numRows() == 0) echo "OK"; else while ($row = $res_users->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[username]<br />"; $errors++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Aliases without Sites")?></th>
        <td><?php if ($res_aliases->numRows() == 0) echo "OK"; else while ($row = $res_aliases->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[leftpart]<br />"; $errors++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Users having higher Quota than valid")?></th>
        <td><?php if ($res_maxquota->numRows() == 0) echo "OK"; else while ($row = $res_maxquota->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[username]@$row[sitename]<br />"; $errors++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Sites which MaxQuota is not set")?></th>
        <td><?php if ($res_maxquota0->numRows() == 0) echo "OK"; else while ($row = $res_maxquota0->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[name]<br />"; $warnings++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Sites which MaxUsers is not set")?></th>
        <td><?php if ($res_maxaddr0->numRows() == 0) echo "OK"; else while ($row = $res_maxaddr0->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[name]<br />"; $warnings++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Sites which MaxAliases is not set")?></th>
        <td><?php if ($res_maxaliases0->numRows() == 0) echo "OK"; else while ($row = $res_maxaliases0->fetchRow(DB_FETCHMODE_ASSOC)) { echo "$row[name]<br />"; $warnings++; }?></td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Sites containing more aliases than valid")?></th>
        <td>
            <?php
                $start_errors = $errors;
                ...
                if ($start_errors == $errors) echo 'OK';
            ?>
        </td>
    </tr>
    <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
    <tr valign="top">
        <th><?php echo $tl->get("Checking for Sites containing more users than valid")?></th>
        <td>
            <?php
                $start_errors = $errors;
...
<br /><br />
<?php
    if ($errors > 0) echo '<p style="color: white;">'. sprintf($tl->get('The XAMS-Database is not fully consistent (%s errors)!'), $errors). '</p>';
    if ($warnings > 0) echo '<p style="color: white;">'. sprintf($tl->get('You should think about limiting MaxQuota/MaxUsers/MaxAliases on Site-Level (%s warnings).'), $warnings). '</p>';

    if ($errors == 0 && $warnings == 0) echo '<p style="color: white;">'. $tl->get('The XAMS-Database seems to be OK.'). '</p>';
?>
<?php } ?>
