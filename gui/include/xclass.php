...
class xclass
{
    ...
    var $VSPECfile = null;
    var $VSPEC = null;
    var $pid_field_name = null;
    var $formular_error = array();
    var $lngbase = null;
    var $objhaschanged = false;

    var $formular_errors = false;

    var $authenticated = false;
    var $auth_mode = _AUTH_NONE;


    function xclass($init=true)
    {
        // DB connection
        ...
        // VSPEC
        if (empty($this->VSPECfile)) $this->VSPECfile = strtolower(get_class($this));
        $filename = 'include/vspec/'. $this->VSPECfile. '.inc';
        ...
        if ($init && is_array($this->VSPEC))
            foreach ($this->VSPEC as $k=>$elem)
            {
                ...
                if ($this->VSPEC[$k]['type'] == Tpid) $this->pid_field_name = $k;
            }
    ...
    function insertId($table, &$id, $before)
    {
        if ($before) return;
    ...
    function Authenticate($mode=false)
    {
        if ($mode)
    ...
    function setAuthMode($mode=_AUTH_NONE)
    {
        $this->auth_mode = $mode;
    }
    ...
    function isAuthUpdate()
    {
        return ($this->auth_mode & _AUTH_UPDATE);
    }

    function isAuthDelete()
    {
    ...
    // Logs XAMS actions
    function XAMS_Log($MsgType, $Message, $MsgStatus='ok', $TopID=false, $UserType=false)
    {
        if (!_LOG_OPTION || (_LOG_LEVEL < 3 && $MsgType == 'Selection'))
            return false;

        $logctables = array('unknowns', 'users', 'customers', 'resellers', 'admins');
        ...
        $result = $this->db->query('INSERT INTO pm_log (id, msgtype, msgstatus, resource) VALUES (?, ?, ?, ?)', array($LogID, $MsgType, $MsgStatus, 'XAMS'));
        ...
        if ($field_index > 0)
            $this->db->query('INSERT INTO ! VALUES (?, ?)', array($logctable, $LogID, $TopID));
        else
            $this->db->query('INSERT INTO ! VALUES (?)', array($logctable, $LogID));

        if (!empty($Message))
        {
            $Message .= $this->Object2Log();
            $this->db->query('INSERT INTO pm_log_message (logid, name, message) VALUES (?, ?, ?)', array($LogID, $username, $Message));
        }
    }

    // Generates bitfield from comma seperated string
    function string2bitfield($str, $trans)
    {
        $arr = explode(',', $trans);
        $bitfield = 0;
        $i = 1;
        foreach ($arr as $elem)
        {
            if (preg_match("/$elem/", $str)) $bitfield |= $i;
            $i *= 2;
        }
        return $bitfield;
    }
    ...
    function bitfield2string($bitfield, $trans)
    {
        $arr = explode(',', $trans);
        $i = 1;
        $str = null;
    ...
    {
        foreach ($vals as $k) if ($k == $val) return true;
        return false;
    }

    // Generates SQL-Insert for this object
    function create_sql_insert($table_name, &$val_arr)
    {
        $sql = "INSERT INTO $table_name (COLS) VALUES (VALS)";
        $vals = $cols = $val_arr = array();
        $vals2 = null;
        foreach ($this->VSPEC as $k=>$elem)
        {
            if (
        ...
        $sql = preg_replace(array('/COLS/', '/VALS/'), array(implode(', ', $cols), $vals2), $sql);
        return $sql;
    }

    // Generates SQL-Update for this object
    function create_sql_update($table_name, &$val_arr)
    {
    ...
    function create_sql_delete($table_name, &$val_arr)
    {
        $sql = "DELETE FROM $table_name WHERE id = ?";
        $val_arr = array($this->{$this->pid_field_name});
        return (count($val_arr) == 0) ? null : $sql;
    }

    function Add()
    {
        if (!$this->isAuthAdd()) die($this->objname. '->Add() - Permission denied.');
    ...
    function Update()
    {
        if (!$this->isAuthUpdate()) die($this->objname. '->Update() - Permission denied.');
    ...
    function Delete()
    {
        if (!$this->isAuthDelete()) die($this->objname. '->Delete() - Permission denied.');
    ...
    function ObjectChanged()
    {
        ...
        if ($changed) $this->assign('changed', 'true');
        return $changed;
    }

    // Superclass Load() to write Database-Results in $this->
    function Load($sql, $val)
    {
        ...
        if ($row > 0)
            ...
            {
                $k = strtolower($k);
                if (isset($this->VSPEC[$k]) && $this->isin($this->VSPEC[$k]['type'], array(Tnumeric,Tpid,Tid,Tbitfield)))
                    $elem = (int)$elem;
    ...
    function assignVSPEC($arra, $arrb, $value)
    {
        $this->VSPEC[$arra][$arrb] = $value;
    }

    function assignFormVar($var, $value)
    {
        $this->{$var} = $value;
    ...
    function Assign2Object($data)
    {
        foreach ($data as $elem)
        {
            ...
            if (!is_array($this->VSPEC) || isset($this->VSPEC[$elem])) $this->Vars4FormChecking[] = $elem;
        }
    }

    function error_func($k, $text)
    {
        if (!isset($this->formular_error[$k]['error'])) $this->formular_error[$k]['error'] = null;
        $this->formular_error[$k]['status'] = $this->formular_errors = true;
        $this->formular_error[$k]['error'] .= $text;
    }

    function show_field_property($name)
    {
        if (isset($this->formular_error[$name])
            && isset($this->formular_error[$name]['status'])
            && $this->formular_error[$name]['status'] == true)
            return '<img src="'. _SKIN. '/img/error.png" alt="" title="'. $this->formular_error[$name]['error']. '" />';
        else
            return '&nbsp;';
    }
    ...
    function check_formular($form_mode)
    {
        ...
        foreach ($this->Vars4FormChecking as $k)
        {
            ...
            if ($empty_ok)
            {
                ...
                {
                    case Tid:
                        if (!preg_match('/^[1-9]\d*$/', $http) && $http != 0 ) $this->error_func($k, $this->i18n->get('Invalid ID'));
                        break;
                    case Tnumeric:
                        ...
                        {
                            // Length ok?
                            if ((isset($elem['maxl'])) && (strlen($http) > $elem['maxl'])) $this->error_func($k, sprintf($this->i18n->get("Too long (max='%s')"), $elem[maxl]));
                            if ((isset($elem['minl'])) && (strlen($http) < $elem['minl'])) $this->error_func($k, sprintf($this->i18n->get("Too short (min='%s')"), $elem['minl']));

                            // Value ok?
                            if ((isset($elem['max'])) && ((int)$http > $elem['max'])) $this->error_func($k, sprintf($this->i18n->get("Value is too big (max='%s')"), $elem['max']));
                            if ((isset($elem['min'])) && ((int)$http < $elem['min'])) $this->error_func($k, sprintf($this->i18n->get("Value is too low (min='%s')"), $elem['min']));

                            // String contains only 0-9?
                            if (!preg_match('/-?\d/', $http)) $this->error_func($k, $this->i18n->get('Contain non-numeric Characters'));
                        }
                        break;
                    case Tpassword:
                    case Talphanumeric:
