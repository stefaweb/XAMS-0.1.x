...
class Sites extends xclass
{
    ...
    function check_formular($form_mode)
    {
	    ...
	    //    $this->error_func('resellerid', $this->i18n->get('Select a Reseller in the drop-down menu'));
            ...
            if ($form_mode == 'new' && $this->myReseller->FreeSites() == 0)
                $this->error_func('resellerid', $this->i18n->get('Unfortunately no more sites can be added. Site-Quota of this Reseller has reached.'));
    ...
    function ApplyQuota()
    {
        ...
        $free_sitequota = $this->myReseller->FreeQuota();
        if ($free_sitequota < 0)
        {
        ...
        else
        {
            ...
            {
                if ($this->maxquota <= $this->LoadedData['maxquota'])
                    $free_sitequota = $this->maxquota;
                else
                    $free_sitequota += $this->LoadedData['maxquota'];
            }
