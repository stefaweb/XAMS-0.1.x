    ...
    <xsl:template match="menuroot/menu">
        ...
        <head>
            ...
            #userinfo {
                <xsl:if test="usertype = 'administrator'">
                    background-image: url(<xsl:value-of select="$skindir"/>/img/menu/user_info_admin.png);
                    font-weight: bold;
                </xsl:if>
                <xsl:if test="usertype = 'reseller'">
                    background-image: url(<xsl:value-of select="$skindir"/>/img/menu/user_info_reseller.png);
                </xsl:if>
                <xsl:if test="usertype = 'customer'">
                    background-image: url(<xsl:value-of select="$skindir"/>/img/menu/user_info_customer.png);
                </xsl:if>
                <xsl:if test="usertype = 'user'">
                    background-image: url(<xsl:value-of select="$skindir"/>/img/menu/user_info_user.png);
                </xsl:if>
            }
            </style>
        ...
        <body-->
            ...
            <table id="menutable" cellspacing="0" cellpadding="0">
                ...
                <tr id="version">
                    <td>
                        <img src="{$skindir}/img/logo.png" width="88" height="59" alt="XAMS" title="eXtended Account Management System" onclick="window.open('http://www.xams.org');" onmouseover="this.style.cursor = 'pointer';" />
